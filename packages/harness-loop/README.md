...
作るときに判定の口を渡すと、道具の実行の前に判定を挟みます。
だめだったときは、道具を動かさずに理由を結果として返します。

記録の相手も渡していれば、判定の期間 `mg.gate` は、道具の期間
`mg.tool` の兄弟として `mg.harness` の下に並びます。
だめだった呼び出しには、`mg.tool` は生まれません。

判定の口を渡さなければ、いまと同じ動きのままです。
