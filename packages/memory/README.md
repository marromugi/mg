...
1 つの保存の操作は、同時に呼んでも結果が変わりません。
呼んだ順に 1 つずつ行ったときと、同じ結果になります。
これは `MemoryStore` の約束です。
どの実装も、この約束を守ります。

- 同時に呼んだ書き込みが同じ版を渡したら、先に呼んだ側が書きます。
  後の側は、衝突のエラーで失敗します。
- 触る項目が重ならず版も合えば、同時に呼んだ書き込みはどちらも保存されます。
- 書き込みの後に呼んだ読み出しは、その書き込みの結果を読みます。

この約束が及ぶのは、1 つの保存に対する呼び出しです。
それより広く守る範囲は、実装ごとに決めます。
守る範囲は、実装ごとにこの文書へ書きます。
...
この実装は、同時の呼び出しの約束を広げます。
同じプロセスで同じファイルを開いた保存どうしにも、約束が及びます。

- 同じプロセスの中では、同じファイルへの操作を 1 つずつ順に行います。
  開く関数の中のマイグレーションも、対象に入ります。
- 同じファイルかどうかは、ファイルのデバイスと inode で見分けます。
  別の名前で開いても、指す先が同じファイルなら対象になります。
  シンボリックリンクとハードリンクと、大文字小文字違いの名前が当たります。
- `":memory:"` で開いた保存は、開くたびに別の保存先です。
  その保存の中だけで、呼び出しを 1 つずつ順に行います。
- 前の操作が失敗しても、後に呼んだ操作は順に行われます。

この範囲を超える取り合いは、約束の外です。
ファイルは、この保存だけのものです。
別の種類の保存とファイルを共有することは支えません。
共有すると、記憶の書き込みの途中に相手が書くことがあります。
その相手は、次の待ちで失敗します。

別のプロセスが同じファイルの書き込みのロックを持つときもあります。
そのときは、5 秒待った後に SQLite の busy のエラーで失敗します。
そのエラーは、そのまま呼び出し元に届きます。
