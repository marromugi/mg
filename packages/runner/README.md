...
`run` は、作業場を開いた後にこの並びを渡します。
渡す先は `createSubagent` です。
組み立てたサブエージェントを、ハーネスに渡します。
渡す第 2 引数も、`run` が用意します。
自分で `createSubagent` を呼ぶのは、他の使い方が要るときだけです。
...
「親が選ぶ」で出どころの名前が重なったときも、組み立ての時点で例外になります。
親のワークスペースの名前との重なりも、同じ扱いです。
