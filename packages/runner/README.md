...
例外の文には、作業場の名前と、排他的に持つものの名前が入ります。
...
runner は、設定からサブエージェントを組み立てます。
設定の型は `SubagentConfig` です。
組み立てる関数は `createSubagent` です。
...
| `tools`       | いいえ   | 子が使うツールです。                                            |
| `gate`        | 条件つき | 子の中の呼び出しを判定します。`tools` を書いたら必須です。      |

`tools` を書いて `gate` を書かない設定は、型の検査に落ちます。
どちらも書かない設定は、そのまま通ります。

設定に書かなかったものは、子に渡りません。
プロバイダーやゲートやツールを、親の設定から補うことはしません。

`createSubagent` の入力のスキーマは、文字列 `prompt` だけを持ちます。
...
`harness` の設定が誤っていると、`createSubagent` を呼んだ時点で例外になります。
走行の途中ではなく、組み立ての時点で確かめるためです。

走行の設定 `RunConfig` に、サブエージェントの並びはまだ書けません。
