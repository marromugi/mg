...
どちらも走行にそのまま渡ります。signal はトリガーにも渡ります。
...
6. 判定の記録を閉じて書き出します。
7. 決めた id を渡し、`run` で走行を始めて完了を待ちます。

返す値は、発火したかどうかで形が変わります。

- 発火しなかったとき: 判定のセッションの id と判定です。
- 発火したとき: 判定のセッションの id と判定と、走行の結果です。走行の結果は `run` の返り値そのままです。
...
走行が失敗すると、走行のエラーをそのまま投げます。
判定の記録は、走行への参照を持ったまま残ります。
