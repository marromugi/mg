...
| `system`   | いいえ | 走行の先頭に置く system です。   |
| `messages` | はい   | 新しいメッセージです。           |

`system` は走行にだけ渡り、会話には保存しません。
`messages` は 1 件以上要ります。system のメッセージは書けません。

入口は、1 回の呼び出しにつき次の順で動きます。

1. 中断の signal がすでに中断済みなら、その理由で拒否します。会話は読みません。
2. 指定の範囲で会話を読みます。読み出しが失敗したら、そのエラーをそのまま投げます。
3. system（あれば）と、読んだエントリーのメッセージと、新しいメッセージを、この順に並べます。
