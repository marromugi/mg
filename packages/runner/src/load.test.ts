...
describe("loadRun", () => {
  test("resolves the default export of a valid config", async () => {
    ...
    expect(config.harness).toEqual({ kind: "loop", model: "m", maxTurns: 1 });
  });

  test("rejects with InvalidRunConfigError when there is no default export", async () => {
    await expect(loadRun("src/__fixtures__/no-default-export.ts")).rejects.toThrow(
      InvalidRunConfigError,
    );
  });

  test("names the path and the missing field when harness is absent", async () => {
    await expect(loadRun("src/__fixtures__/missing-harness.ts")).rejects.toThrow(
      /src\/__fixtures__\/missing-harness\.ts.*harness/,
    );
  });

  test("propagates import failures for a missing file", async () => {
    await expect(loadRun("src/__fixtures__/does-not-exist.ts")).rejects.not.toThrow(
      InvalidRunConfigError,
    );
  });

  const distLoadPath = fileURLToPath(new URL("../dist/load.js", import.meta.url));
