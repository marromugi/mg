...
const validate = (path: string, value: unknown): RunConfig => {
  if (!isObject(value)) throw new InvalidRunConfigError(path, "default export must be an object");

  if (typeof value.name !== "string" || value.name.length === 0) {
    throw new InvalidRunConfigError(path, "name must be a non-empty string");
  }
  ...
  if (typeof value.provider.generate !== "function") {
    throw new InvalidRunConfigError(path, "provider.generate must be a function");
  }
  if (typeof value.provider.stream !== "function") {
    throw new InvalidRunConfigError(path, "provider.stream must be a function");
  }
  ...
  if (typeof value.harness.kind !== "string") {
    throw new InvalidRunConfigError(path, "harness.kind must be a string");
  }
